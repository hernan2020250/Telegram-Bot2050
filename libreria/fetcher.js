...
exports.createExif = (pack, auth) =>{
    ...
    fs.writeFileSync('./misstks/data.exif', Buffer.concat([_, __, ___, ____]), function (err) {
        console.log(err)
        if (err) return console.error(err)
        return `./misstks/data.exif`
    })
...
exports.modStick = (media, client, mek, from) => {
    ...
    try {
        console.log(media)
        spawn('webpmux', ['-set','exif', './misstks/data.exif', media, '-o', out])
        .on('exit', () => {
